...
export default function SalesPage() {
  ...
  const handleConfirmBill = async (billId: string) => {
    ...
    try {
      const res = await api.post(`/bills/${billId}/confirm`);
      setSuccessMsg(`Invoice #${res.data.bill_number} confirmed and finalized successfully! Stock deducted & Sales voucher posted.`);
      await fetchBills();
    } catch (err: any) {
      console.error(err);
      alert(err.response?.data?.detail || "Failed to confirm bill");
    } finally {
  ...
  return (
    <>
      <div className="no-print">
        ...
        {/* Review Status Queue Tab Navigation */}
        <div style={{ display: "flex", gap: "8px", marginBottom: "16px", borderBottom: "1px solid var(--border)", paddingBottom: "12px" }}>
          <button
            onClick={() => setReviewTabFilter("confirmed")}
            className={reviewTabFilter === "confirmed" ? "btn-primary" : "btn-secondary"}
            style={{
              padding: "8px 16px",
              fontSize: "0.875rem",
              borderRadius: "8px",
              ...
              alignItems: "center",
              gap: "8px",
              background: reviewTabFilter === "confirmed" ? "#0284c7" : undefined,
            }}
          >
            <CheckCircle2 size={16} color={reviewTabFilter === "confirmed" ? "#fff" : "#38bdf8"} />
            Confirmed Invoices ({confirmedBills.length})
          </button>

          <button
            onClick={() => setReviewTabFilter("under_review")}
            className={reviewTabFilter === "under_review" ? "btn-primary" : "btn-secondary"}
            style={{
              padding: "8px 16px",
              fontSize: "0.875rem",
              borderRadius: "8px",
              ...
              alignItems: "center",
              gap: "8px",
              background: reviewTabFilter === "under_review" ? "#d97706" : undefined,
              borderColor: pendingReviewBills.length > 0 ? "rgba(245, 158, 11, 0.5)" : undefined,
            }}
          >
            <Clock size={16} color={reviewTabFilter === "under_review" ? "#fff" : "#f59e0b"} />
            Under Review / Orders
            {pendingReviewBills.length > 0 && (
              <span
                style={{
                  background: reviewTabFilter === "under_review" ? "#fff" : "#f59e0b",
                  color: reviewTabFilter === "under_review" ? "#d97706" : "#000",
                  padding: "2px 7px",
                  borderRadius: "10px",
                  fontSize: "0.75rem",
                  fontWeight: 700,
          ...
          <button
            ...
            className={reviewTabFilter === "all" ? "btn-primary" : "btn-secondary"}
            style={{
              padding: "8px 16px",
              fontSize: "0.875rem",
              borderRadius: "8px",
                ...
                {filteredBills.map((bill) => {
                  ...
                  >
                    ...
                    <td>
                      ...
                      {(!bill.is_reviewed_by_admin || bill.status === "under_review") && (
                        <span
                          className="badge"
                          style={{
                    ...
                    <td style={{ textAlign: "right" }}>
                      <div style={{ display: "flex", gap: "6px", justifyContent: "flex-end", alignItems: "center" }}>
                        {/* Admin Confirm & Finalize Button for Under Review Orders */}
                        {isAdmin && (!bill.is_reviewed_by_admin || bill.status === "under_review") && (
                          <button
                            ...
                            title="Confirm & Finalize Invoice (Deduct Stock & Post Sales)"
                          >
