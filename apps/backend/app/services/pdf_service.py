...
def generate_bill_pdf(bill: Bill, tenant: Tenant, paper_format: str = "a4") -> bytes:
    """Generate a GST-compliant Tax Invoice PDF in memory (Supports A4 and Half-A4 / A5)"""
    is_a5 = str(paper_format).lower() in ["a5", "half_a4", "half-a4", "half_page"]
    selected_pagesize = A5 if is_a5 else A4
    margins = 16 if is_a5 else 30
    ...
    title_font_size = 14 if is_a5 else 18
    title_leading = 16 if is_a5 else 22
    body_font_size = 7.5 if is_a5 else 9
    body_leading = 9.5 if is_a5 else 12
    table_font_size = 7 if is_a5 else 8
    table_leading = 8.5 if is_a5 else 10
    ...
    header_col_widths = [210, 177] if is_a5 else [280, 250]
    header_data = [
        [
            Paragraph(f"<b>{tenant.business_name}</b><br/>GSTIN: {tenant.gst_number or 'Unregistered'}<br/>Authorized Tax Invoice", subtitle_style),
            Paragraph(f"<b>TAX INVOICE</b><br/>Invoice No: <b>{bill.bill_number}</b><br/>Date: {bill.created_at.strftime('%d-%m-%Y')}", title_style)
        ]
    ]
    ...
    story.append(Spacer(1, 8 if is_a5 else 15))
    ...
    bill_to_data = [
        [
            ...
            Paragraph(f"<b>Payment Details:</b><br/>Mode: <b>{'CREDIT' if bill.payment_mode == 'credit' else 'CASH'}</b><br/>Status: <b>{bill.payment_status.upper()}</b><br/>Place of Supply: {'Inter-state (IGST)' if bill.is_interstate else 'Intra-state (CGST+SGST)'}", subtitle_style)
        ]
    ]
    ...
    bill_to_table.setStyle(TableStyle([
        ...
        ('PADDING', (0, 0), (-1, -1), 5 if is_a5 else 8),
        ('VALIGN', (0, 0), (-1, -1), 'TOP'),
    ]))
    ...
    items_col_widths = [16, 136, 32, 40, 42, 42, 35, 44] if is_a5 else [25, 175, 45, 55, 60, 65, 45, 60]
    items_table = Table(table_rows, colWidths=items_col_widths)
    items_table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor("#1e3a8a")),
        ('TEXTCOLOR', (0, 0), (-1, 0), colors.white),
        ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
        ('FONTSIZE', (0, 0), (-1, 0), 7 if is_a5 else 8),
    ...
    story.append(Spacer(1, 6 if is_a5 else 12))
